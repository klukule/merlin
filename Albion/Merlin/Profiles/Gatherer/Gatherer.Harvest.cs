...
namespace Merlin.Profiles.Gatherer
{
    public sealed partial class Gatherer
    {
        ...
        public void Harvest()
        {
            ...
            {
                _localPlayerCharacterView.CreateTextEffect("Harvest()" + " | "
                    + System.Convert.ToString(_localPlayerCharacterView.GetLocalPlayerCharacter().GetHealth().GetValue()) + "/" + System.Convert.ToString(_localPlayerCharacterView.GetLocalPlayerCharacter().GetHealth().GetMaximum()) + " | "
                    + System.Convert.ToString(previousPlayerInfo.StuckCount) + " | "
        ...
        public void HarvestHarvestableObjec(HarvestableObjectView resource)
        {
            ...
            if (centerDistance >= minDistance)
            {
                ...
                {
                    //Core.lineRenderer.positionCount = pathing.Count; // - dTormentedSoul
                    //Core.lineRenderer.SetPositions(pathing.ToArray()); // - dTormentedSoul
        ...
        public void HarvestMob(MobView mob)
        {
            ...
            var weaponItem = _localPlayerCharacterView.LocalPlayerCharacter.sl().o();
            var isMeleeWeapon = weaponItem == null || weaponItem.bu() == Albion.Common.GameData.AttackType.Melee;
            var attackRange = _localPlayerCharacterView.LocalPlayerCharacter.jw() + mob.Mob.we().eu();
